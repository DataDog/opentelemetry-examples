...
Kubernetes Explorer's **Installation Updates Recommended** panel reports on the Datadog Agent components it expects to find in a cluster:

| Column | Populated by |
| :----------------------- | :----------- |
| Agent versions           | the Datadog Agent |
| Cluster Agent versions   | the Datadog Cluster Agent |
| Kubernetes State Metrics | the Cluster Agent's [`kubernetes_state_core`][17] check, which emits `kubernetes_state.*` metrics |
| Orchestrator             | the orchestrator check — or, in this deployment, the Datadog exporter's `orchestrator_explorer` |

The deployment in this guide runs neither Agent, so a healthy cluster reports `NONE`, `NONE` and `DISABLED` for the first three columns and `ENABLED` for `Orchestrator`: the Datadog exporter sends the same resource payloads the Agent's orchestrator check does, and nothing in this deployment emits `kubernetes_state.*`.

`DISABLED` here means the Agent's kube-state-metrics *check* is absent, not that kube-state-metrics *data* is missing. The cluster collector scrapes kube-state-metrics directly and ships those series under their original `kube_*` Prometheus names. Kubernetes Explorer itself is populated by the [`k8sobjects`][3] receiver and is unaffected.

#### Confirm kube-state-metrics data is arriving

Query any of the metrics that only exist when the scrape is working, scoped to your cluster:

```
k8s.pod.count{kube_cluster_name:<YOUR CLUSTER NAME>}
kube_pod_info{kube_cluster_name:<YOUR CLUSTER NAME>}
```

`k8s.*.count` are generated by the [count][9] connector from kube-state-metrics series, and `kube_*` are the scraped series themselves. The cluster collector also forwards Prometheus' `up` series, which is `1` while the kube-state-metrics scrape is healthy.

If those metrics are missing, work back through the pipeline:

1. **Check the scrape target.** The cluster collector scrapes `kube-state-metrics.default.svc:8080`. `helm install kube-state-metrics prometheus-community/kube-state-metrics` installs into whichever namespace is current, so confirm where the Service actually landed and update the `prometheus` receiver's `static_configs` target to match:

   ```sh
   kubectl get svc -A -l app.kubernetes.io/name=kube-state-metrics
   ```

2. **Check the collector is accepting the scrape.** The collector exposes its own telemetry on port `8888`:

   ```sh
   kubectl port-forward deploy/otel-cluster-collector 8888:8888
   curl -s localhost:8888/metrics | grep -E 'otelcol_(receiver_accepted|receiver_refused|exporter_send_failed)_metric_points'
   ```

3. **Check `k8s.cluster.name` is set.** Without it the series arrive untagged by cluster and will not match a `kube_cluster_name` filter. See the note under [Install the Collectors](#install-the-collectors).
...
[17]: https://docs.datadoghq.com/integrations/kubernetes_state_core/
