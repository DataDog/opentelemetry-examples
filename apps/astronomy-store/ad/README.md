...
`context_keys` is optional; recognized values are `telescopes`, `binoculars`, `books`, and `planetariums`. Unrecognized or missing keys fall back to a default set of ads. The service returns a JSON array of `Ad` objects:
