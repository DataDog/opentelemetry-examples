...
--tag datadog/opentelemetry-examples:calendar-java-20251215 \
...
docker buildx build \
--platform linux/amd64,linux/arm64 \
--tag datadog/opentelemetry-examples:calendar-java-dd-20251215 \
...
```
helm install -n otel-ingest calendar-otel-java ./deploys/calendar/ --set image.repository=datadog/opentelemetry-examples --set image.tag=calendar-java-20251215,node_group=ng-1
```

Install calendar in K8s with DD SDK

```
helm install -n otel-ingest calendar-dd-java ./deploys/calendar-dd/ --set image.repository=datadog/opentelemetry-examples --set image.tag=calendar-java-dd-20251215,node_group=ng-1
```
